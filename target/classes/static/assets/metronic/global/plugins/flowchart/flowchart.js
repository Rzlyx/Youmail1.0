...
(function() {
  ...
  FlowChart.prototype.render = function() {
    ...
    this.paper.setSize((maym * scale) + (lineWidth * scale), (maxY * scale) + (lineWidth * scale));
    this.paper.setViewBox(0, 0, maym + lineWidth, maxY + lineWidth, true);
  };
  ...
  Symbol.prototype.drawLineTo = function(symbol, text, origin) {
    ...
    if ((!origin || origin === 'bottom') && isOnSameColumn && isUnder) {
      ...
      this.bottomStart = true;
      symbol.topEnd = true;
      maym = bottom.x + (bottom.x - symbolTop.x)/2;
    ...
    if (!this.chart.maymFromLine || (this.chart.maymFromLine && maym > this.chart.maymFromLine)) {
      this.chart.maymFromLine = maym;
    }
  };
