	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		function updateBoxSize() {
			...
			if (needsBox) {
				...
				if (!box) {
					boym = mathRound(-alignFactor * padding) + crispAdjust;
					boxY = (baseline ? -baselineOffset : 0) + crispAdjust;

					wrapper.box = box = shape ?
						renderer.symbol(shape, boym, boxY, wrapper.width, wrapper.height, deferredAttr) :
						renderer.rect(boym, boxY, wrapper.width, wrapper.height, 0, deferredAttr[STROKE_WIDTH]);
	...
	if (isXAxis) {
		...
		each(this.series, function (series, i) {
			if (series.useMapGeometry) {
				...
				dataMax = Math.max(dataMax, pick(series.maym, dataMin));
...
seriesTypes.map = extendClass(seriesTypes.scatter, merge(colorSeriesMixin, {
	...
	getBox: function (paths) {
				...
				if (!point._foundBox) {
					while (i--) {
						if (typeof path[i] === 'number' && !isNaN(path[i])) {
							if (even) { // even = x
								pointMaym = Math.max(pointMaym, path[i]);
					...
					point._midX = pointMinX + (pointMaym - pointMinX) * 
					...
					point._maym = pointMaym;
					point._minX = pointMinX;
					point._maxY = pointMaxY;
					point._minY = pointMinY;
					point.labelrank = pick(point.labelrank, (pointMaym - pointMinX) * (pointMaxY - pointMinY));
					point._foundBox = true;
				}

				maym = Math.max(maym, point._maym);
				minX = Math.min(minX, point._minX);
				maxY = Math.max(maxY, point._maxY);
				minY = Math.min(minY, point._minY);
				minRange = Math.min(point._maym - point._minX, point._maxY - point._minY, minRange);
				hasBox = true;
			}
		});
		...
		if (hasBox) {
			...
			this.maym = Math.max(maym, pick(this.maym, -MAX_VALUE));

			// If no minRange option is set, set the default minimum zooming range to 5 times the 
			// size of the smallest element
			if (xAxis && xAxis.options.minRange === undefined) {
				xAxis.minRange = Math.min(5 * minRange, (this.maym - this.minX) / 5, xAxis.minRange || MAX_VALUE);
			}
